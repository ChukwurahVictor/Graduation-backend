<template>
    <div class="admin-wrapper">
        <el-row>
          <el-button class="btn-add" type="danger" @click="addClick">新增就业计划<i class="el-icon-plus el-icon--right"></i>
          </el-button>
        </el-row>
        <el-table :data="showPlanList"
                  border
                  style="width: 100%">
            <el-table-column prop="admission_vintage" label="招生计划年份" align="center"></el-table-column>
...
<script type="text/ecmascript-6">
  export default {
    ...
    methods: {
      ...
      editClick(index, row) {
        // 编辑 跳转到编辑页面
        this.$router.push({name: 'createPlan', params: row});
      },
      deleteClick(index, row) {
        // 删除
        console.log(index, row);
        this.$confirm('确定删除该条招生信息？', '提示', {
      ...
      changeList(cur, pageSize) {
        this.showPlanList = this.planList.slice((cur - 1) * pageSize, cur * pageSize);
      }
    }
  }
</script>
